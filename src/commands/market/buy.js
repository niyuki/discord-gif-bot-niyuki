...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(balanceUser < priceItem) { message.reply(await client.translate(`
            ...
            message.channel.send(new MessageEmbed().setColor(config.panel.embedColor).setFooter(await client.translate(`Enjoy this until ${moment().add(7, 'd').format('LLLL')} UwU`, message)).setDescription(await client.translate(`**You** have just bought <@&${idItem}> \`${nameItem}\` for **${priceItem}** coins... Enjoy this until`, message)).setTimestamp())
            client.rmv(message.author.id, priceItem)
            message.member.roles.add(idItem)
            setTimeout(() => {
                message.member.roles.remove(idItem)
            }, 1000*60*60*24*7);
