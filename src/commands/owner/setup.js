...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.reply(question).then(x => x.delete({ timeout: 30000}))
        ...
        message.guild.roles.create({
          ...
          ;
          message.channel.send(rolesembed).then(x => x.delete({timeout: 120000}))
          ...
          message.author.send(dmembed)//.catch(() => message.channel.send(await client.translate(`Your DM is closed.. Please Open them and type \`${client.prefix}setrl dm\` to resend the IDs `, message)))

          if(args[0] === 'dm') {
            message.author.send(dmembed)
                //.catch(() => message.channel.send(await client.translate(`Your DM is still closed dude.. Do not make fun of this and please open this seriously. Then type \`${client.prefix}setrl dm\` to resend the IDs `, message)))
          }
        }, 30000);
        setTimeout(() => {
