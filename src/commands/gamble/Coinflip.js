...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        //betcheck
        let bet = args[0];
        if(!args[0]) return message.channel.send(moneyhelp).then(x => x.delete ({ timeout: 5000}))
        arg1 = args[0];
		if(Number === args[0]){
			bet = parseInt(args[0]);		
		}
        if(bet=="all") bet = 50000 || await client.bal(message.author.id)
			...
			if (bet > 50000) {
                message.channel.send(betmax).then(x => x.delete ({ timeout: 5000}))
				return;
			} else if (bet==0) {
                message.channel.send(cf0).then(x => x.delete ({ timeout: 5000}))
				return;
			} else if (bet<0){
                message.channel.send(cfminus).then(x => x.delete ({ timeout: 5000}))
				return;
            }
        ...
        if(await client.bal(message.author.id)==0) {
            message.channel.send(moneyno).then(x => x.delete ({ timeout: 5000}))
        }
		if(bet==0){
			message.channel.send(cf0).then(x => x.delete ({ timeout: 5000}))
			return;
			...
			message.channel.send(choiceundefined).then(x => x.delete ({ timeout: 5000}))
			return;
		}
        ...
        if(bet > 50000) return message.channel.send(betmax).then(x => x.delete ({ timeout: 5000}))
        
        if(await client.bal(message.author.id) < bet) return message.channel.send(moneymore).then(x => x.delete ({ timeout: 5000}))
        ...
        let edited = await message.channel.send(text)
			setTimeout(function(){
				edited.edit(text2)
			},3500);
    }
