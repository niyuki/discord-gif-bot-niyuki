...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!color) return message.channel.send(colorbad).then(x => x.delete ({ timeout: 5000}))
        color = color.toLowerCase()
        if(!money) return message.channel.send(moneyhelp).then(x => x.delete ({ timeout: 5000}))
        if(money > balance) return message.channel.send(moneymore).then(x => x.delete ({ timeout: 5000}))
        ...
        else return message.channel.send(colorbad).then(x => x.delete ({ timeout: 5000 }));
